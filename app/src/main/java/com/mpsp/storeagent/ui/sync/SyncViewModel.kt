...
class SyncViewModel(initialState: SyncState) : MavericksViewModel<SyncState>(initialState) {
    ...
    private fun createDecreaseQuantityParams(decreaseTrainingPhrases: ArrayList<ArrayList<String>>, products: List<Product>) {
        val finalDecreaseQuantityTrainingPhrases = arrayListOf<Intent.TrainingPhrase>()
        val phraseParts: ArrayList<Intent.TrainingPhrase.Part> = arrayListOf()
        ...
        products.forEach { product ->
            quantityIntentions.forEach { intention ->
                decreaseTrainingPhrases.forEach { decreaseTrainingPhrase ->
                    decreaseTrainingPhrase.forEach { partOfPhrase ->
                        if(partOfPhrase.contains("@product@")) {
                            phraseParts.add(
                                Intent.TrainingPhrase.Part.newBuilder()
                                    .setText(partOfPhrase.replace("@product@", product.name))
                    ...
                    finalDecreaseQuantityTrainingPhrases.add(
                        Intent.TrainingPhrase.newBuilder().addAllParts(
                            phraseParts
                        ).build()
                    )
        ...
        if(finalDecreaseQuantityTrainingPhrases.isEmpty()) {
            setIsLoading(false)
            triggerSyncFinishedEvent(false)
            return
        }
        ...
        try {
            ...
            response.intentsList.forEach { intent ->
                if (intent.displayName.equals("basket-decrease-product-value")) {
                    finalDecreaseQuantityTrainingPhrases.addAll(intent.trainingPhrasesList)

                    val intentBuilder = intentClient.getIntent(intent.name).toBuilder()
                    intentBuilder.addAllTrainingPhrases(finalDecreaseQuantityTrainingPhrases)
                    val intent: Intent = intentBuilder.build()
